@extends('layouts.app')

@section('content')
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    Estado de Devoluciones
                    @can('status.create')
                        <a href="{{ route('status.create') }}" class="btn btn-sm btn-primary float-right">Crear</a>                        
                    @endcan
                </div>

                <div class="card-body">
                    <table id="datatables" class="table table-striped table-bordered" style="width:100%">
                        <thead>
                            <tr>
                                <th width="10px">ID</th>
                                <th>Nombre</th>
                                <th>Descripcion</th>
                                <th>Acciones</th>
                            </tr>   
                        </thead>

                        <tbody>
                            @foreach($status as $statu)
                                <tr>
                                    <td>{{ $statu->id }}</td>
                                    <td>{{ $statu->name }}</td>
                                    <td>{{ $statu->description }}</td>
                                    <td>
                                        <div class="btn-group float-right" role="group">
                                        @can('status.show')
                                        <button type="button" class="btn">
                                            <a href="{{ route('status.show', $statu->id) }}" class="btn btn-sm"><i class="fa fa-eye"></i></a>
                                        </button>
                                        @endcan

                                        @can('status.edit')
                                        <button type="button" class="btn btn-warning">
                                            <a href="{{ route('status.edit', $statu->id) }}">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                         </button>
                                        @endcan
                                        
                                        @can('status.destroy')
                                        {!! Form::open(['route' => ['status.destroy', $statu->id], 'method' => 'DELETE']) !!}
                                            <button class="btn btn-lg btn-danger" onclick="return confirm('Estas Seguro eliminar Estado {{ $statu->id }}')">
                                                <i class="fa fa-trash" style="color: blue"></i>
                                            </button>
                                        {!! Form::close() !!}
                                        @endcan
                                    </div>
                                        
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                    {{ $status->render() }}
                </div>
            </div>
        </div>
    </div>
</div>
@endsection
